...
test('a guest can fill out and submit the RSVP form', async ({ page, context }) => {
  await page.goto('/rsvp.html');

  const [formPage] = await Promise.all([
    context.waitForEvent('page'),
    page.locator('.rsvp-cta').click(),
  ]);
  ...
  await expect(formPage.getByText(/response has been recorded/i)).toBeVisible({ timeout: 15000 });
});
